...
class AOC(object):
        ...
        if not force and os.path.exists(path):
            ...
            if req.status_code != 200:
                ...
                if content:
                    ...
                    print('Error when trying to download data from', url, ': finded file is empty.')
        return self
    ...
    def analyse(self):
        if self.brut:
            ...
            print(
                f'{empty_line_amount} empty line(s) found. Analyse as {"monline" if self.monoline_mode else "multiline"} data.')

            if self.monoline_mode:
                stripped = map(lambda x: x.strip(), self.brut.strip().split('\n'))
                if self.numeric_mode:
                    self.data = list(map(int, filter(lambda x: x.isdigit(), stripped)))
                else:
                    self.data = list(filter(lambda x: x, stripped))
            else:
                stripped = map(lambda x: x.strip(), self.brut.strip().split('\n\n'))
                if self.numeric_mode:
                    self.data = list(map(lambda x: list(map(int, filter(lambda y: y.isdigit(), x))), stripped))
                else:
                    self.data = list(
